...
| D2 | **Scaling claim**: run clean 10v5 **or** scope RQ1 to equal-capacity | run **#185** (cheap) | #185 |
...
## 1. Open tasks — prioritised, with launch prompts

### Tier 1 — thesis-critical, low-risk (do first; independent, run in parallel)
...
**#182 · T1.3 — Coupled-loop integration test**
...
### Tier 2 — fixes/refactors (medium risk)

**#183 · T2.1 — Consolidate the autoscaler controllers**
...
**#188 · T4.1 — PPO: retire or train+deploy**
...
### Track B / future work (only if Track A is done + time remains)
...
> After the thesis-writing agent finishes AND #185/#186/#187 have landed: read all of `thesis/report/chapters/` + the abstract/conclusion, and reconcile against today's evidence — fold in `audit/METHODOLOGY.md`, `audit/LIMITATIONS.md`, the `adaptive-advantage` results (PR #184) + the ablation table, and the `audit/REPORT.md` failure analysis. Produce `audit/THESIS_RECONCILIATION.md` first (chapter → currently-claims → now-true → backing-artifact → keep/update/add), confirm the central claim is **Track A**, then apply the edits to the chapters. Flag any place today's results *contradict* the draft.
...
## 5. Suggested order
1. **You:** settle D1–D5 (§0).
2. Parallel: **#181, #182, #186, #187** (independent, low-risk).
3. **#183** → then **#185** (10v5 clean once the flap is fixed).
4. **#188** (PPO retire) per D3.
5. *(Track B, optional)* **#189** → **#190**.
6. **§3 thesis reconciliation** — last, after the thesis agent + #185/#186/#187.
