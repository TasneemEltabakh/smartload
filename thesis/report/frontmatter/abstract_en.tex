...
% TODO(vps-rerun): pool range + scaling-action count from fresh VPS adaptive batch
The deployed gradual-degradation detector closes a failure mode that point-based
rules miss, lifting its detection score from zero to \BManFone{} on slow drift. For
routing, the deployed deterministic capacity-aware heuristic holds SLA
violations well below the round-robin baseline; a trained policy is kept dormant
in shadow mode, where it proves round-robin-equivalent and so is reported as an
honest null result rather than a learned-routing win.
