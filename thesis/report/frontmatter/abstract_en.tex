...
This thesis presents SmartLoad, a lightweight middleware that brings traffic
routing, demand forecasting, anomaly detection, and proactive scaling into one
decision loop, with a deterministic safety fallback on every action. SmartLoad
...
the backend pool grew automatically from one to six instances across about
twelve scaling actions, then shrank again as demand fell, with the deterministic
fallback ready on every step. The anomaly model agrees about 91.4\% with the
deterministic baseline while also catching extra variance. The learned routing
policy performs on par with the strong, industry-standard round-robin baseline
in shadow mode, and a guaranteed-safe fallback keeps routing at least as
effective as that established approach.
