...
\begin{table}[htbp]
  \centering
  \caption[SmartLoad services: language, port, and role]{SmartLoad services: language, default port, and role.}
  ...
  \begin{tabularx}{\linewidth}{@{}l l c Y@{}}
    ...
    resource-collector & Python             & sidecar & Ships per-container CPU and memory telemetry to the metrics store. \\
    ...
    anomaly-detector & Python               & 8082    & \texttt{trend\_rule} detector (default) over a threshold fallback, with a selectable Isolation Forest engine \cite{liu2008isolationforest}. \\
    forecasting      & Python               & 8083    & \texttt{harmonic\_residual} forecaster (default) over a moving-average fallback, with an optional ARIMA$(2,0,2)$ model \cite{box2015timeseries}. \\
    rl-engine        & Python               & 8084    & Deployed \texttt{monotone} routing heuristic (live) with a dormant shadow-only PPO comparator \cite{schulman2017ppo}. \\
...
\begin{table}[htbp]
  \centering
  \caption[Shared infrastructure components]{Shared infrastructure components.}
  \label{bm:tab-infra}
  \small
  \begin{tabularx}{\linewidth}{@{}l c Y@{}}
    ...
    Grafana \cite{grafana}         & 3000 & Renders the operational overview dashboard; routing, anomaly, scaling, and forecast dashboards are planned. \\
    \bottomrule
  \end{tabularx}
\end{table}
...
\begin{table}[htbp]
  \centering
  \caption[Principal third-party Python libraries]{Principal third-party Python libraries.}
  \label{bm:tab-libs}
  \small
...
Exact pinned library versions are listed in each service's
...
deployment-specific and are omitted here, because they depend on
...
\begin{table}[htbp]
  \centering
  \caption[Self-hosted, open-source cost model]{Self-hosted, open-source cost model (figures are deployment-specific).}
  \label{bm:tab-cost}
  \small
  \begin{tabularx}{\linewidth}{@{}l l Y@{}}
    ...
    Container host (1 VM or owned server) & Recurring & deployment-specific \\
    Storage for telemetry hypertables    & Recurring & deployment-specific \\
    Electricity (self-hosted hardware)   & Recurring & deployment-specific \\
    One-time model-training compute       & One-time  & deployment-specific \\
    Training datasets                     & One-time  & \$0 (public, openly licensed traces). \\
    Engineering / operator labour         & Recurring & deployment-specific \\
    \bottomrule
  \end{tabularx}
\end{table}
...
\begin{table}[htbp]
  \centering
  \caption[Principal HTTP endpoints]{Principal HTTP endpoints (see the OpenAPI spec for full schemas).}
  \label{bm:tab-api}
  \small
  \begin{tabularx}{\linewidth}{@{}l l Y@{}}
    \toprule
...
\begin{table}[htbp]
  \centering
  \caption[Redis control-bus channels]{Redis control-bus channels with publishers and subscribers.}
...
The abridged rule above is the single-step reference. The full deployed rule is
the \texttt{target} controller, with its capacity sizing, clamp, dead-band, and
asymmetric cooldowns; it is given in \cref{bm:alg:autoscaler}.
...
% =====================================================================
\chapter{Promoted Engine Algorithms}
\label{bm:app-algorithms}

This appendix gives the full internal logic of the four engines the running
system deploys by default: the trend-rule anomaly detector, the harmonic-residual
forecaster, the monotone capacity-aware router, and the forecast-driven target
controller. Each section pairs a faithful pseudocode listing with the parameters
and defaults drawn from the service source and the deployed configuration. The
narrative treatment, with the per-engine decision flowcharts, is in
\cref{ch:impl_decision}.

\input{backmatter/alg_trend_rule}
\input{backmatter/alg_harmonic}
\input{backmatter/alg_monotone}
\input{backmatter/alg_autoscaler}
...
The \texttt{safe\_mode} recovery runbook and the backup and restore procedure for
the TimescaleDB volume are maintained as operational documentation alongside the
deployment.
