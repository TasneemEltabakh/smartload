...
develops the concrete architecture --- the three-plane separation, the
...
peak as latency or errors. When one backend in the pool degrades --- a slow disk,
a warm-up stall, a noisy co-tenant on a shared host --- a blind scheduler keeps
sending it its share of traffic, and every request unlucky enough to land there is
penalised. The conventional remedy, permanently over-provisioning the pool so that
peaks and sick instances are absorbed by slack, trades a reliability problem for a
cost-and-energy problem: capacity is paid for and powered continuously to serve
load that arrives only occasionally.

The engineering problem this project addresses is therefore: \emph{how can the
routing and capacity decisions at the front door of a backend pool be made
...
\begin{table}[ht]
\centering
\caption{Project scope. The right-hand column lists deliberate Phase~2 deferrals,
which are scope decisions oriented toward a future managed, multi-customer
offering --- not unfinished work within the present phase.}
\label{dsa:tab:scope}
\small
...
\begin{table}[ht]
\centering
\caption{Standards and conventions adopted, and the concrete design artefact that
incorporates each.}
...
route --- policy read and write (\texttt{GET}/\texttt{POST /api/v1/policy}), the
...
distinct standards. Per-\emph{request} telemetry --- the per-backend latencies,
...
collector rather than a bespoke agent. Per-\emph{process} health --- each service's
own liveness, cycle counters, and decision counters --- is published separately in
...
channels. These channels --- \texttt{smartload.policy} (policy updates),
...
are split into functional requirements --- what the system must \emph{do}, organised
around the capabilities the prototype delivers --- and non-functional requirements
--- the qualities every part of the system must \emph{exhibit}, of which safety is
first among equals. Each requirement is numbered so that the architecture of
...
\begin{table}[ht]
\centering
\caption{Functional requirements. Each derives from the problem statement and names
the deterministic baseline it must degrade to.}
\label{dsa:tab:fr}
\small
...
\begin{table}[ht]
\centering
\caption{Non-functional requirements. Safety (NFR-1) is the dominant quality; the
remainder follow from the problem statement and the contextual concerns.}
\label{dsa:tab:nfr}
\small
\renewcommand{\arraystretch}{1.25}
...
standard format, and a uniform \texttt{/health} endpoint; every adaptive decision is
...
carries an explicit safety clause --- a reconfiguration must never empty the pool ---
because dropping the last healthy backend is exactly the kind of edge case a
...
\begin{table}[ht]
\centering
\caption{Project constraints, mapped onto the accreditation constraint vocabulary
\cite{abet2025criteria}.}
...
a single shared module for message envelopes, a single shared module for SQL --- so
that the system remains comprehensible and changeable by a small team; ad-hoc JSON and
...
chapters that follow. The schedule constraint --- one year, three people --- is the
...
\begin{table}[ht]
\centering
\caption{Principal project risks and their mitigations. Each risk is paired with a
mitigation that is built into the design, so that every entry reads as foresight the
architecture already answers rather than a problem left open.}
\label{dsa:tab:risks}
\small
\renewcommand{\arraystretch}{1.25}
