...
\end{tikzpicture}
\caption{The structural lag in reactive scaling. An autoscaler can act only after a
metric crosses its threshold, and the requested instance becomes useful only after
container start and cache warm-up. The interval between the breach and useful
capacity, bounded below by start-plus-warm-up at roughly 30 to 90 seconds
for typical web workloads \cite{verma2015borg}, is exactly the interval over which
the spike is absorbed as degraded service. This is the gap a forecast-driven scaler
is designed to close by committing capacity before the threshold is crossed.}
\label{dsa:fig:lag}
\end{figure}
...
\begin{tikzpicture}[
  ...
  \draw[link] (c) -- (p4);
\end{tikzpicture}
\caption{The four design principles that make an adaptive decision safe to place in
the request path. They are mutually supporting: a deterministic floor and a stateless
path together make rerouting on failure free, a uniform plugin contract makes the
floor swappable for a learned upgrade, and the three together yield graceful
degradation. Each principle is traced to a concrete requirement later in the chapter.}
...
offering rather than unfinished work within the present phase.}
\label{dsa:tab:scope}
\small
...
\begin{tikzpicture}[
  ...
  \draw[arr] (b2) -- (b3);
\end{tikzpicture}
\caption{Two backend-degradation mechanisms that a blind round-robin scheduler and
NGINX's passive health check both miss, because in each case the backend keeps
returning \texttt{200~OK} while only its latency degrades. (a)~A persistent latency
penalty keeps a backend in rotation and lifts the pool's tail latency. (b)~A latency
spike that still returns success never trips the \texttt{max\_fails} counter. FR-2
exists to detect both from latency and error-rate telemetry and exclude the degraded
backend that the classical data plane would otherwise keep serving.}
