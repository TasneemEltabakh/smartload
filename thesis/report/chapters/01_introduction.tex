...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{@{}l Y l@{}}
    ...
    \midrule
    RQ1 & On identical infrastructure (same NGINX, backend pool, and workload), does enabling the SmartLoad decision plane produce measurably better tail-latency and failure-rate outcomes than plain NGINX round-robin? & System bench \\
    \addlinespace
    ...
    \addlinespace
    RQ3 & Does the load-balancer sidecar pull a misbehaving backend out of rotation faster than NGINX's passive health checks, when the backend is slow but still returns 200 OK so passive checks never trip? & System bench, anomaly phase \\
    \addlinespace
