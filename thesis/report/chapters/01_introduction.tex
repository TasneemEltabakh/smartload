...
\begin{table}[ht]
  \centering
  \caption{What the current single-tenant phase delivers, and what is deferred to the Phase~2 managed, multi-customer offering.}
...
\begin{table}[ht]
  \centering
  \caption{The four research questions and the evaluation surface that answers each. Wording follows the project specification.}
  \label{tab:intro:rqs}
  \small
...
\begin{tikzpicture}[node distance=7mm and 8mm]
  ...
  \node[slnode] (p1) {\textbf{Anomaly-aware}\\\textbf{routing}\\Isolation Forest\\flags sick\\backends out};
  \node[slnode,right=of p1] (p2) {\textbf{Forecast-driven}\\\textbf{autoscaling}\\predict demand,\\grow / shrink\\the pool ahead};
  \node[slnode,right=of p2] (p3) {\textbf{Learned (shadow)}\\\textbf{routing}\\PPO ranks\\backends;\\sidecar ignores\\until promoted};
...
\end{tikzpicture}
\caption{The three learned pillars of SmartLoad and the deterministic floor beneath them. Anomaly-aware routing, forecast-driven autoscaling, and a shadow-mode learned router all run off the request path; each degrades onto a simple, predictable baseline, and the \texttt{safe\_mode} switch pins the whole system to that floor on demand. This expresses the central discipline that routing may learn while safety does not.}
