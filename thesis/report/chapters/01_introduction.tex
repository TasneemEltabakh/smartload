...
\slpitch{\textbf{SmartLoad} is a self-hosted middleware that inserts a
telemetry-driven decision layer between client traffic and a pool of backend
services. It keeps a classical NGINX load balancer as its data plane and adds an
out-of-band control plane that detects misbehaving backends, forecasts demand,
learns a routing policy from real traffic, and scales the pool ahead of
saturation, all behind a \texttt{safe\_mode} switch that pins every engine to its
...
detector, which lifts gradual-degradation F1 from 0 to 0.845 where stateless
...
the deployed trend-rule engine), the two agree about 91.4\% of the time, with the
