...
earlier \texttt{baseline-vs-smartload} bench, whose 50-user closed loop tied: that
workload sat below the queue knee, so a degraded backend slowed without ever shedding
and the slowdown hid in the worst-case maximum rather than separating the two sides.
That tie is a benchmark artefact, not a real equivalence, and is reported as such. The
...
channel detects it and the sidecar pulls it out. Second, anomalies are injected
...
stale-\texttt{down} confounds so the 5v5 A/B measures pure routing and exclusion. The
slow-but-healthy \texttt{D\_slow} backend that keeps returning \texttt{200 OK} is the
companion case NGINX's passive \texttt{max\_fails} check also cannot solve. The finding
this bench produces is discussed in \cref{imc:theory} and quantified in
\cref{tab:imc:phases-obs}.
...
target. On the agreement sweep it covers a $16 \times 16 = 256$-cell grid and shows
broad agreement with the threshold rule after the model
was re-calibrated in production-shape feature space; the precise agreement rate and
per-cell verdict counts await the host-migration benchmark batch. The direction of the
disagreements is what matters here and is discussed below.
...
\begin{table}[H]
  \centering
  \caption[Anomaly-engine comparison over the feature grid]{Anomaly-engine comparison: threshold rule against Isolation Forest over a $16\times16=256$-cell latency-by-error-rate grid. Per-cell verdict counts and the exact agreement rate await the host-migration benchmark batch; the qualitative shape of the comparison is summarised here.}
  \label{tab:imc:anomaly}
  \small
  \begin{tabular}{@{}l l@{}}
    \toprule
    \textbf{Property} & \textbf{Observation} \\
    \midrule
    Overall agreement & broad (most cells share a verdict) \\
    Disagreement direction & all \texttt{healthy}$\rightarrow$\texttt{unhealthy} (value-add) \\
    Relaxations (\texttt{unhealthy}$\rightarrow$\texttt{healthy}) & none \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{figure}[H]
  ...
  \caption[Anomaly agreement heatmap]{Anomaly agreement heatmap, where the Isolation Forest engine broadly agrees with the threshold rule and the only disagreements are in the value-add direction, with no relaxed verdicts.}
...
rule. Reporting the realised pool range and per-run scaling-action count against the
capacity model is left to future work, pending the end-to-end adaptive benchmark run
under the equal-capacity scope.
...
The broad agreement with zero relaxed verdicts is that prediction realised: the
...
the trained PPO routing policy \emph{matches} NGINX round-robin: both record the same
round-robin-equivalent reward and a zero SLO-violation rate,
