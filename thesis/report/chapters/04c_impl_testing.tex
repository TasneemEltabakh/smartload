...
\emph{baseline-vs-smartload routing bench} compares plain NGINX round-robin against the
full decision plane on an identical stack (RQ1, RQ3); and the
\emph{anomaly-engine-bench} compares the threshold rule against the trained Isolation
Forest engine across a feature grid. All three share one discipline: every run writes a
...
The adaptive-bench drives the live stack through a five-phase Locust~\cite{locust} load
shape that exercises both forecast-driven scale-out and anomaly-driven reroute in a
single repeatable command. The phases, summarised in \cref{tab:imc:phases-plan}, are:
\texttt{A\_bootstrap} (a low warm-up setting a forecast baseline); \texttt{B\_forecast\_burst}
(a sharp ramp to 200 users the forecaster should see coming before saturation);
\texttt{C\_sustain} (the burst held while a phase-D-style anomaly is injected into one
backend); \texttt{D\_anomaly\_scale\_down} (load dropped so the autoscaler sheds
capacity); and \texttt{E\_steady} (a low tail confirming the system settles). An
...
engaged, and contrasts the two latency distributions. Its design principle is to
minimise hidden variables. The compose stack, backend pool image, NGINX configuration,
test-backend code, and operator-UI back-end-for-front-end are byte-for-byte identical
across both sides; only five environment flags differ, and those five are exactly the
switch that turns SmartLoad's intelligence on or off. \Cref{tab:imc:protocol} reproduces
the diff between the two recorded environment profiles in
\texttt{experiments/baseline-vs-smartload/env/}.
...
\texttt{200 OK}, chosen so NGINX's passive \texttt{max\_fails} health check cannot
trivially solve the problem. That keeps the comparison fair: plain round-robin keeps
dispatching a fifth of its traffic to the slow backend, the failure class the anomaly
and lb-sidecar pipeline is designed to address. The finding this bench produces is
discussed in \cref{imc:theory}.
...
is measured and no sampling error remains to bound.
...
predicts: the pool grows under the forecast burst and sustain phases, contracts on the
scale-down phase, and settles in the steady tail without flapping, while the controller
holds error at zero in the steady phases and sheds load in a controlled way when offered
demand exceeds the host-capped pool rather than collapsing into a 502 cascade. The
headline magnitudes for this run (per-phase tail latency and error rate, realised pool
range, and scaling-action count) are being regenerated from a fresh benchmark batch and
...
before an actuation, up to the cooldown-bounded maximum for the earliest bootstrap-phase
forecasts that must wait for the burst to cross a decision band. The measured delay
...
arrived during the forecast burst and sustain phases, and scale-in followed cleanly on
the scale-down phase, confirming the loop closes end-to-end.
...
the policy correctly learned round-robin equivalence. That tie, though, is a tie on a
simulator that cannot tell routing policies apart. The closed-loop benchmark, which lets
a routing choice affect the next state, does separate them, and there the trained
policy is the \emph{worst} of the contenders: the deployed \texttt{monotone} heuristic
violates the objective on \BMrlMonoSla{} of the run, round-robin on \BMrlRrSla{}, and
the shipped PPO policy on \BMrlPpoSla{}. The deployed router is therefore the
...
plainly: the routing acceptance gate (a clear delta on at least three of five metrics)
is currently \emph{red} on this PPO model, and closing it requires retraining on a
heterogeneous-latency distribution, under a closed-loop simulator, before a learned
policy could be promoted to \texttt{active}.

\paragraph{The cost of an active rewrite.} The routing bench also surfaces a cost the
project reports rather than hides. On the heterogeneous run the per-phase p95 figures
track closely between the two sides, but the single maximum latency across the run is
higher on the SmartLoad side. That gap is not a routing regression; it is the
lb-sidecar's NGINX-reload cost during the anomaly window, the brief connection blip an
\texttt{upstream.conf} rewrite incurs. It is included unfiltered because suppressing it
