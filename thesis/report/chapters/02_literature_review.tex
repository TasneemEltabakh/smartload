...
active health checking. The important point is that none of the three ships a
built-in feedback loop that re-weights upstreams from external telemetry. In
NGINX, dynamic upstream weights need either a paid module, a configuration
rewrite followed by a reload, or an external operator process driving the
change. The data plane in all three projects is designed around in-memory
static configuration plus passive health probing, so ``intelligent'' routing is
...
lineage of cluster-management systems --- Borg, Omega, and Kubernetes
\cite{burns2016borgomega} --- whose evolution shaped the reactive-scaling model
in use today. The defining feature of this model is that it is
\textbf{reactive}: a metric crosses a threshold, a stabilisation window
prevents flapping, and only then does the replica count move. The lag between
load arriving and a new replica being ready is bounded below by container start
time plus warm-up, often tens of seconds for typical web workloads --- a gap
also visible in the large-scale fleet measurements that motivated the Borg work
\cite{verma2015borg}. Cloud autoscalers therefore react \emph{after} load has
already risen, so the spike is felt by users before capacity arrives.
Predictive variants exist in proprietary offerings, but they usually keep the
forecaster and the per-request scheduler decoupled, so the prediction cannot
inform routing.

SmartLoad's answer is to make prediction first-class and to couple it to
actuation. A forecasting service publishes a short-horizon request-rate
forecast, and an autoscaler uses that forecast to grow the backend pool
\emph{ahead} of saturation, falling back to a reactive sliding-window rule when
the forecaster is silent or unhealthy. The contribution is not a new
autoscaling algorithm but the proactive-by-default posture with a reactive
safety net.
...
time-series telemetry --- per-backend latency, error rate, and request count
--- lands in TimescaleDB \cite{timescaledb}, a PostgreSQL extension whose
...
reactive autoscaler; a service mesh represents the heavyweight, policy-rich
routing fabric. The table makes the wedge clear: SmartLoad is the only column
that combines predictive scaling and learned routing with a deterministic
fallback and an operator override, and it does so at a far lower operational
weight than a mesh.
...
\begin{tabular}{@{}lcccc@{}}
\toprule
\textbf{Capability} & \textbf{SmartLoad} & \textbf{NGINX alone} & \textbf{K8s HPA} & \textbf{Service mesh} \\
\midrule
...
Operational weight                    & Low--moderate & Low & Moderate & High \\
\bottomrule
\end{tabular}
\end{table}
...
across the whole system is: \emph{can a load balancer be made to adapt to
changing backend conditions --- predicting load and learning routing
preferences --- without giving up the deterministic guarantees that make
classical load balancing trustworthy?} Each subsection below covers the slice
of the literature relevant to one engine and records what that literature
establishes and where its findings stop short of an integration-ready system.
...
is wrong --- which is what motivates both the forecasting engine and its
mandatory reactive fallback.
...
models --- which is why SmartLoad ships a moving-average baseline as a
...
labels, and it serialises to a small artifact --- all properties that fit a
...
methods miss, and --- importantly for evaluation --- supplies a public dataset
with per-timestep ground-truth labels. SmartLoad uses that labelled dataset not
for training but to check detector quality against a fixed F1 gate. The field's
...
Proximal Policy Optimization (PPO) \cite{schulman2017ppo} is the workhorse: its
clipped surrogate objective turns off the gradient when the policy update leaves
a trust region, which makes it forgiving of mis-tuned learning rates without an
explicit constraint --- the property that makes it practical for a small team.
SmartLoad uses the action-masking PPO variant from Stable-Baselines3
\cite{raffin2021sb3} so that unhealthy backends are removed from the action
space before the policy ever samples, a hard safety property rather than a
learned preference.
...
place. The learned router runs in shadow mode by default, the classical
scheduler stays the trusted floor, and the anomaly constraints filter the
action space before the policy is ever consulted. This keeps the adaptive
component bounded by guarantees the operator can rely on while the policy is
exercised against real traffic. Extending the learned policy's advantage on
varied workloads --- richer state features, online updating, and a wider set of
replayed traces --- is an active research direction the project is pursuing.

\subsection{Autonomic computing and control loops}
\label{lit:sec:research-mapek}

SmartLoad's overall shape --- observe telemetry, decide, actuate, repeat --- is
an instance of the autonomic-computing vision set out by Kephart and Chess
\cite{kephart2003autonomic}, in which self-managing systems take low-level
...
The HTTP control surface --- the operator API for reading state and posting
...
conventions: semantic versioning, applied consistently to the HTTP API, the
policy schema, the Redis envelope, and the client SDK so that consumers can
reason about compatibility; and the twelve-factor application methodology, whose
discipline around configuration-in-environment, stateless processes, and
disposability maps cleanly onto a containerised, horizontally-composed service
set. For the eventual rollout of learned routing into live traffic, the
progressive-delivery patterns embodied by tools such as Flagger \cite{flagger}
--- canary analysis, automated promotion, and automated rollback gated on
metrics --- are the reference model SmartLoad's shadow- and hybrid-mode rollout
follows.

There is, however, an open opportunity the project targets directly: there is
\textbf{no settled standard for ``learned routing with a safety fallback.''} The
observability and API layers are standardised, and progressive delivery
standardises \emph{how} a new behaviour is rolled out, but neither one says
\emph{what guarantees} an adaptive routing decision must satisfy before it is
trusted, nor how a deterministic fallback should be composed with a learned
policy. SmartLoad fills that space with its own contract --- a strict routing
decision hierarchy in which anomaly constraints are applied first, learned
recommendations second, and the classical scheduler third, with an operator kill
switch that short-circuits all adaptation --- though this contract is a project
convention, not an external standard. Naming this opening is itself a
contribution of the survey: the safety semantics of adaptive routing are an area
where practice currently runs ahead of standardisation.
...
General Data Protection Regulation (GDPR) \cite{gdpr2016}, whose principles ---
...
IP addresses or request headers --- for example, to support consistent-hash
routing on a client attribute --- that data could become personal data, and the
...
recommendation is considered. The system is designed to fail safe, not just to
...
hierarchy follow from the evidence, is the principle in action.

\paragraph{Transparency and operator agency.}
Both codes value transparency and respect for the people affected by a system.
SmartLoad encodes this in two mechanisms: an operator override --- a single
kill-switch flag that immediately disables all adaptation and reverts to
deterministic routing --- and an audit log that records every decision the
system actuates, surfaced through a transparent decision feed. Together the
...
inspectable after the fact --- the Code's emphasis on honesty and accountability
expressed in working machinery.
...
plane --- anomaly detection, forecasting, and learned routing --- coordinated
...
Three openings identified by the survey define the contribution. First,
\textbf{predict ahead instead of react}: SmartLoad drives scaling from a
short-horizon forecast and routing from current-state prediction, moving
capacity before the spike rather than after the alarm, with a reactive rule as
the floor. Second, \textbf{deterministic fallback as a first-class design
property}: every engine ships with a dependency-free baseline and degrades to it
cleanly, so the system never enters a ``no decision available'' state and never
loses its deterministic guarantee. Third, \textbf{the absence of a standard for
safe learned routing}: the survey found mature standards for telemetry, APIs,
and progressive delivery, but none for the safety semantics of an adaptive
routing decision, and SmartLoad's routing hierarchy is offered as a concrete
filling of that gap pending an external standard. The chapters that follow
develop each of these into the system's design and evaluation.
