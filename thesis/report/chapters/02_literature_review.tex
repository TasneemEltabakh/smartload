...
\begin{table}[H]
\centering
\caption[Classical load-balancing scheduling rules]{Classical load-balancing scheduling rules as published by NGINX
\cite{nginx}, HAProxy \cite{haproxy}, and Envoy \cite{envoy}, each deciding from in-memory state alone.}
...
In short, SmartLoad does not replace NGINX. It runs NGINX as its data plane and
adds an out-of-band sidecar that consumes Redis-published routing decisions,
rewrites \texttt{upstream.conf}, and triggers \texttt{nginx -s reload} on the
order of seconds. The classical scheduler stays in place as the deterministic
safety floor and is never deprecated.
...
\begin{table}[H]
\centering
\caption[Production autoscaler shapes and their triggers]{Production autoscaler shapes, where the trigger is reactive in every case except AWS predictive scaling.}
...
and adaptive, capacity-aware routing with a deterministic fallback and an
operator override, and it does so at a far lower operational weight than a mesh.
Its routing is a monotone capacity-aware heuristic, with a learned PPO variant
evaluated in shadow rather than promoted to live traffic.

\begin{table}[H]
\centering
\caption[SmartLoad against the closest alternatives]{SmartLoad compared with the closest alternatives across the axes relevant to predictive, safety-bounded load management.}
...
Adaptive (capacity-aware) routing     & Yes (heuristic; learned variant in shadow) & No & No & Policy-driven, not adaptive \\
...
\begin{table}[H]
\centering
\caption[Related-work lineage by SmartLoad engine]{Related-work lineage: the research foundations grouped by the SmartLoad engine each informs.}
...
ARIMA as the field's classical reference baseline; the principle of a simple
model with uncertainty intervals over a moving-average floor, realised in
SmartLoad by a harmonic-residual default. \\
\addlinespace
Anomaly detection &
...
Policy-gradient routing with a clipped objective, evaluated as a shadow
comparator; the hard action-masking safety property is realised by the deployed
monotone policy and anomaly filtering, not the dormant PPO. \\
\addlinespace
Autonomic computing &
...
finding the field offers is that, at short horizons, a well-specified classical
or even a moving-average predictor is often competitive with much heavier
sequence models. That finding is why SmartLoad deploys a lightweight
harmonic-residual predictor as its default forecaster, keeps a moving-average
baseline as the never-fails fallback floor, and offers an ARIMA model as an
optional classical alternative, rather than reaching straight for a deep model.
...
but expensive to train and serve, which is why SmartLoad ships an interpretable
trend-rule detector as its default, keeps a static threshold rule as the
dependency-free fallback floor, and treats the heavier Isolation Forest model as
a guarded opt-in upgrade.
...
plane covering anomaly detection, forecasting, and adaptive capacity-aware
routing (a monotone heuristic, with a learned PPO variant evaluated in shadow
rather than promoted), coordinated over a Redis control bus and grounded in a
conventional, standards-based observability stack. The decision plane is composed with the data plane through
